...
if [[ -L "$HOME/zsh" ]]; then
  ...
  for module in aliases exports functions asdf; do
...
if command -v git &> /dev/null; then
  ...
  if git config --global commit.gpgsign | grep -q "true"; then
    ...
    if check_file "$HOME/.gitconfig.local" "Git local config (for signing key)"; then
      if grep -q "signingkey" "$HOME/.gitconfig.local"; then
        check_pass "Git signing key is configured in ~/.gitconfig.local"
      else
        check_warn "Git signing key not found in ~/.gitconfig.local"
      fi
    fi
  else
...
if [[ -n "$SSH_AUTH_SOCK" ]]; then
  ...
  if ssh-add -L &> /dev/null; then
    ...
    check_warn "SSH agent is configured but no keys are loaded"
  fi
else
